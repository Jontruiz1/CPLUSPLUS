...
//
// Cache-friendly Matrix Multiplication app
//
// Multiplies in a smarter way to be more cache-friendly, and thus efficient.  For more details, see
// http://www.cs.rochester.edu/~sandhya/csc252/lectures/lecture-memopt.pdf
//
// Usage:
//   MatrixMultiply.exe [-?] [-n MatrixSize]
//
...
int main(int argc, char *argv[])
{
	...
	cout << "** Cache-friendly Matrix Multiply Application **" << endl << endl;
	...
	double** C = MatrixMultiply(A, B, _matrixSize);
...
void ProcessCmdLineArgs(int argc, char* argv[])
{
	for (int i = 1; i < argc; i++)
	{
		...
		{
			cout << "**Usage: MatrixMultiple.exe [-?] [-n MatrixSize]" << endl << endl;
			exit(0);
		}
