			...
			if (!inputVect.empty() && smallestLeft(inputVect.front())) {
				...
				isSolvable = true;
					...
					for (int i = 0; i < holdingVects.size(); ++i) {
						...
						if (!holdingVects.at(i).empty() && smallestLeft(holdingVects.at(i).back())) {
							...
							holdingVects.at(i).pop_back();
							isSolvable = true;
							break;
						}
						else if (!inputVect.empty() && (holdingVects.at(i).empty() || (inputVect.front() < holdingVects.at(i).back()))) {
...
private:
	...
	bool smallestLeft(int compVal) {
		if (smallestTrack(compVal, inputVect) /*&& smallestTrack(compVal, outputVect)*/ && smallestHeld(compVal, holdingVects)) {
			return true;
		}
