...
public:
	...
	const Polynomial<NumberType>& operator=(const Polynomial<NumberType>&& rhs);//R-val reference		    //assignment operators +- and use member function binary op + for this class
	...
	void read(istream& in = cin) {
		...
		while (coef != 0) {
			in >> deg;
			term_list.push_back(Monomial<NumberType>(coef, deg));
			in >> coef;
			//insert_in_poly(Monomial<NumberType>(coef, deg));
		}
